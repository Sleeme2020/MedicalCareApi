...
namespace SiteDemo.Service
{
    public class DiseaseProxyType
    {
        ...
        public List<ViewDiseasesType> GetDiseaseType()
        {
            ...
            var httpResult = client.GetAsync("http://localhost:5014/Diseases").GetAwaiter().GetResult().Content.ReadFromJsonAsync<List<ViewDiseasesType>>();
            httpResult.Wait();
            var Result = httpResult.Result;
            if (Result is null) new Exception();
            cache.Set("ALL_DisieseType", Result, new MemoryCacheEntryOptions().SetAbsoluteExpiration(TimeSpan.FromMinutes(35)));
            return Result;
        }
